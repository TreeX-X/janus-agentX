...
- **Costs and limits**: shim 名单为静态集合，归 `windows-shell` Note 所有；行尾恢复取主导风格，混合行尾文件的插入行随主导风格；后台 `JobManager` 与同步 `command.run` 超时/打断路径均为整树预清加句柄杀兜底（`tryTreeKill` 单一出处）；JanusX 壳侧已对齐（`runAdhoc` 元字符守卫、启动失败落盘、`stop` 与同步路径整树强杀，见 JanusX 仓 `2026-09-14-adhoc-shell-parity`）。混合行尾或模糊缩进的高频重试一旦出现，重访模糊策略。
