...
- **Costs and limits**: at most one automatic reopen per run by default; anything needing a second automatic attempt waits on an explicit budget raise at dispatch. The scheduler never repairs infra-level failures (stale baselines, lease loss, malformed receipts) — those stay manual. Desktop adoption, Ink surfacing of auto events, and cross-checkout scheduling remain follow-ups.
