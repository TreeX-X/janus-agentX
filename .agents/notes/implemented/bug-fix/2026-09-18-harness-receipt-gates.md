...
Evidence validation rescans the checkout and can cost repeated scans in larger dependency graphs. Verification across multiple repository identities remains unavailable until a host supplies the other roots. [Portable results](../architecture/2026-09-18-harness-portable-results.md) reconstruct same-repository results in another checkout and prove closeout against current-branch task, receipt and code contents. The [task execution host](../architecture/2026-09-18-harness-task-execution.md) supplies scoped xdo turns and receipts; shared maintenance conversation control and the complete desktop execution flow remain separate integration work.
