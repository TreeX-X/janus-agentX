...
import { prepareTaskTurn, verifyTaskExecution, type TaskVerificationPorts } from '../src/harness/task-execution.js';
...
async function fixture(mode: 'xdo' | 'xflow' = 'xdo', manual = false) {
...
function ports(overrides: TaskVerificationPorts = {}): TaskVerificationPorts {
  return {
    ...
    review: async (input) => ({ review: { kind: 'self', verdict: 'approved', actor: 'tester', reviewedManifestHash: input.manifestHash },
...
describe('task execution lifecycle', () => {
  ...
  it('requires delegated capability for xflow and explicit manual evidence', async () => {
    ...
    await expect(prepareTaskTurn(delegated.root, delegated.runId, delegated.token)).rejects.toThrow('CAPABILITY_UNAVAILABLE');
