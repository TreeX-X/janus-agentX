...
export function buildChatSystemPrompt(input: SystemPromptBuilderInput): string {
  ...
  // Registry order is host-dependent; sorting here keeps the system prefix stable across turns.
  ...
  return [
    ...
    'Locate an unknown path first with workspace_search mode=files and glob; use flat content search with path/glob filters and regex alternatives for symbols, and pass withContext:true only when the surrounding lines or the file hash is needed for an edit. Batch independent lookups. Prefer search over walking the tree when looking for code, symbols, or text; overview shallow (depth 1) first and deepen only as needed. Read only needed line ranges, and treat returned content plus its hash as current evidence. When workspace_read returns truncated:true, continue with offset=nextOffset until the needed lines are covered; never re-read offset=1 to guess later content.',
